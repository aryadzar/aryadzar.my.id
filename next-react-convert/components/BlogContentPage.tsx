...
import ModernTableOfContents from './table-of-contents'
import { extractFirstImage } from '@/src/utils/thumbnail-ext'
import preprocessHtmlWithZoomWrapper, { renderWithZoom } from '@/src/utils/imageHelperBlog'
...
export default function BlogClient({ title, content, published, image }: Props) {
  ...
  useEffect(()=>{
      const result = preprocessHtmlWithZoomWrapper(content); // ← ini bisa jalan di browser
      const contentsWithIds = addIdsToHeadings(result)
      setEnhancedContent(contentsWithIds);
  }, [])
  ...
  return (
    <div className="min-h-screen bg-[#0a0a18] text-white relative overflow-hidden">
      ...
      {/* Table of Contents */}
      <ModernTableOfContents headings={extractHeadingsFromHtml(enhancedContent)} />
      <motion.div
      ...
      >
        ...
        >
          ...
          >
            <p className="text-gray-400">{new Date(published).toLocaleDateString()}</p>
            <motion.h1
          ...
          >
            {renderWithZoom(enhancedContent)}
          </motion.div>
      </motion.div>
    </div>
  )
